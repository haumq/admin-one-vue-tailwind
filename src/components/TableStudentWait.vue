...
<template>
      ...
      <tr v-for="(student, index) in itemsPaginated" :key="student.Row">
        ...
        <td data-label="Tên" class="lg:whitespace-nowrap">
          ...
          <small class="hidden lg:block">{{ new Date(student.NgaySinh).toLocaleDateString() == 'Invalid Date' ?
            student.NgaySinh : new Date(student.NgaySinh).toLocaleDateString() }}</small>
        ...
        <td class="before:hidden lg:w-1 whitespace-nowrap">
          <BaseButtons type="justify-end" no-wrap>
            <span v-if="student.TrangThai == 1">
              ...
              <BaseButton color="info" :icon="mdiPriorityLow" small @click="transferToLastQueue(currentStudent.Row)" />
              <BaseButton color="danger" :icon="mdiTrashCan" small @click="removeFromQueue(student.Row)" />
            </span>
  ...
  <div class="p-3 lg:px-6 flex justify-end items-center border-t border-gray-100 dark:border-slate-800 select-none" v-if="numPages > 0 && rowsList.length > 0">
      <small class="mr-10">
        <span>Phím tắt: </span>
      ...
      <button @click="transferMultiToFinish(rowsList)"
  ...
  <section>
    <button @click="isModalActive = !isModalActive">âfgagew</button>
